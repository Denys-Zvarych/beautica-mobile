...
/// * [categoryName]
/// * [canReview]
/// * [providerCanReviewClient] - TRUE only for the CURRENT authenticated viewer, and only on GET /bookings/{id}: the viewer has provider review-authority over this booking, its status is COMPLETED, it has a real (non-guest) client, and no ClientReview exists for it yet. FALSE for a CLIENT/SALON_MASTER viewer, an unauthorized provider, or any row served by GET /bookings/me (both the CLIENT and provider listing paths hardcode false — see BookingDetailResponse's class javadoc). Gates the \"Залишити відгук про клієнта\" CTA; the write endpoint (POST /client-reviews) re-checks the same conditions server-side regardless of this value.
...
/// * [awaitingClosure] - Derived, read-time-only (Phase 29.1/29.2) — TRUE when this booking's status is still CONFIRMED but its endsAt has already elapsed: no scheduled job ever transitions such a booking to a terminal state, so this flags the ones the provider still needs to close via /complete, /not-complete or /decline. NEVER persisted, NEVER cached — recomputed on every read from (status, endsAt, the current instant). Orthogonal to canReview: an elapsed-but-unclosed CONFIRMED booking reads TRUE here and FALSE for canReview, since review eligibility keys off closure (COMPLETED), never off elapsed wall-clock time. The same for every row of GET /bookings/me and for GET /bookings/{id} — a pure function of the booking, not of the viewer.
@BuiltValue()
abstract class BookingDetailResponse
  ...
  /// Derived, read-time-only (Phase 29.1/29.2) — TRUE when this booking's status is still CONFIRMED but its endsAt has already elapsed: no scheduled job ever transitions such a booking to a terminal state, so this flags the ones the provider still needs to close via /complete, /not-complete or /decline. NEVER persisted, NEVER cached — recomputed on every read from (status, endsAt, the current instant). Orthogonal to canReview: an elapsed-but-unclosed CONFIRMED booking reads TRUE here and FALSE for canReview, since review eligibility keys off closure (COMPLETED), never off elapsed wall-clock time. The same for every row of GET /bookings/me and for GET /bookings/{id} — a pure function of the booking, not of the viewer.
