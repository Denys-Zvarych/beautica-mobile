...
> ApiResponseVoid requestChangePasswordOtp()
...
try {
    final response = api.requestChangePasswordOtp();
    print(response);
} catch on DioException (e) {
    print('Exception when calling UserControllerApi->requestChangePasswordOtp: $e\n');
}
```
