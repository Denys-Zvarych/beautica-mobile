...
// .dart`'s private `_ProgressHint` (Phase 14.17) so the salon schedule hub's
// footer (`BookingSummaryBar`'s new `progressCount`/`progressTotal` slot,
// this same phase) can render the EXACT SAME "X з Y заплановано" well
// instead of a near-duplicate — REUSE-FIRST: a private widget is promoted,
// never copied. `ScheduleConfirmBar` (the salon per-master "Час" screen's
...
/// See the file header — shared by `ScheduleConfirmBar` (salon "Час" screen,
/// per-master) and `BookingSummaryBar` (salon schedule hub, per-service).
