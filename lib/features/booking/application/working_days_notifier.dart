  ...
  Future<List<WorkingDay>> build(WorkingDaysQuery query) async {
    ...
    return await ref
        ...
        .getWorkingDays(
          ...
          from: query.from,
          to: query.to,
          // Phase 14.20 / MO-2: when the caller (the independent-master
          // booking calendar) supplies serviceIds, the resolved `working` flags
          // become availability-aware (summed duration + existing bookings +
          // now+15min cutoff) instead of schedule-shape. Null for the salon
          // step-3 picker, which stays on the schedule-shape signal.
