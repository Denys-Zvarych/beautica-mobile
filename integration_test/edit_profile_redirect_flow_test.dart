  ...
  Future<GoRouter> bootOnProfile(WidgetTester tester, FakeBackend fb) async {
    final GoRouter router = await AppHarness.boot(
      ...
      extraOverrides: [
        ...
        // cycle-stub-ok: this flow tests the edit-profile redirect path, not the logout cascade — it never invokes a cyclic teardown entrypoint. The auth-cascade cycle is regression-guarded by logout_flow_test.dart, which keeps the REAL graph. (auth is also stubbed here, so no auth→services cycle exists to hide anyway.)
