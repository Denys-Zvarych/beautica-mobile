...
//
// FIXED  → server-formatted priceDisplay passes through ("750 грн").
// RANGE  → reformatted client-side to a hyphenated band ("200 - 600 грн"),
...
void main() {
  group('ServicePriceDisplay.format', () {
    test('FIXED → passes through the server priceDisplay string', () {
      ...
      expect(ServicePriceDisplay.format(s), '750 грн');
    });
    ...
    test('RANGE → hyphenated band, NOT "від … до …"', () {
      ...
      expect(ServicePriceDisplay.format(s), '200 - 600 грн');
    });
    ...
    test('RANGE → whole-hryvnia formatting (no decimals)', () {
      final s = _service(
        ...
        priceMin: 150,
        priceMax: 1200,
        priceDisplay: 'від 150 до 1200 грн',
      );
      expect(ServicePriceDisplay.format(s), '150 - 1200 грн');
    });

    test('FIXED with empty priceDisplay → falls back to "<priceMin> грн"', () {
      final s = _service(priceType: ServicePriceType.fixed, priceMin: 500);
      expect(ServicePriceDisplay.format(s), '500 грн');
    });
    ...
    test('RANGE with null priceMax → falls back to FIXED behaviour', () {
      ...
      expect(ServicePriceDisplay.format(s), '300 грн');
    });
  });
}
