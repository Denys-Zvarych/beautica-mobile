...
void main() {
  ...
  // ── Phase 319 D4 — 400 ServicePriceShapeMismatchFailure names the salon's ──
  // ──   existing price shape ─────────────────────────────────────────────────
  //
  // Phase 315 D3 discriminates this 400 (`data.code ==
  // SERVICE_PRICE_SHAPE_MISMATCH`) from an ordinary validation 400 — that
  // discrimination buys nothing unless the rendered copy actually differs.
  // The generic-ValidationFailure-unchanged case is already covered above
  // ("generic errValidation snackbar and flags no row", line ~2083) — not
  // duplicated here.

  group('400 ServicePriceShapeMismatchFailure names the salon\'s existing '
      'price shape', () {
    Future<void> pumpWithSingleValidRow(WidgetTester tester) async {
      await _pump(
        tester,
    ...
    testWidgets(
      'RANGE shape mismatch (min 400, max 900) names the band in ₴, never '
      '«грн», and does NOT show the generic errValidation snackbar',
      (tester) async {
        // Fixture values differ from every default row/price elsewhere in
        // this file (60/500) so a hardcoded snackbar string could not pass
        // by coincidence (project_fixture_values_can_defang_assertions).
        when(() => h.repo.bulkCreate(any())).thenThrow(
        ...
        // Hardcoded literal (not a call to the same getter production uses)
        // so an ARB template typo can't move both sides of the assertion
        // together — pinned separately in
        // test/l10n/delete_service_blocked_and_price_shape_copy_test.dart.
        const String expected =
            'У салоні ця послуга вже коштує від 400 до 900 ₴. Виберіть такий '
            'самий формат ціни.';
        expect(find.text(expected), findsOneWidget);
        expect(expected, contains('400'));
        expect(expected, contains('900'));
        expect(expected, contains('₴'));
        expect(expected, isNot(contains('грн')));

        // Not the generic message — the discrimination bought something.
        expect(find.text(l10n.errValidation), findsNothing);

        // The batch was rolled back — stay on screen with the selection
        // intact, same house rule as the 409 duplicate case above.
        expect(find.byType(ServiceSetupScreen), findsOneWidget);
        expect(h.pushedRoutes, isEmpty);
    ...
    testWidgets(
      'FIXED shape mismatch (min 400) names a single price, not a range',
      (tester) async {
        when(() => h.repo.bulkCreate(any())).thenThrow(
          const ServicePriceShapeMismatchFailure(
            salonPriceType: ServicePriceType.fixed,
            salonPriceMin: 400,
          ),
        );
        ...
        // Hardcoded literal — see the RANGE case above for why.
        const String expected =
            'У салоні ця послуга вже коштує 400 ₴. Виберіть такий самий '
            'формат ціни.';
        expect(find.text(expected), findsOneWidget);
        expect(expected, isNot(contains('900')));
        expect(expected, isNot(contains('грн')));
