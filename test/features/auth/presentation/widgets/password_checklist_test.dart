...
//
// Batch-2 B3: passwordRules() now requires an AppLocalizations argument so
// that rule labels are served through the l10n pipeline. Tests use
// lookupAppLocalizations(uk) for direct access without a widget tree.
...
// Ukrainian l10n instance used for passwordRules() calls throughout this file.
// lookupAppLocalizations is generated by flutter gen-l10n and does not require
// a widget tree, making it ideal for pure-unit tests.
...
void main() {
  ...
  group('passwordRules factory', () {
    test('1. returns exactly 3 rules (max-128 rule removed in Phase 2.16)', () {
      final rules = passwordRules(_l10n);
      expect(
        rules,
        hasLength(3),
        reason:
    ...
    test('2. rule labels match expected Ukrainian strings (min 8)', () {
      final rules = passwordRules(_l10n);
      // Rule 0 — min length (uses l10n.passwordRuleLength interpolation)
      expect(rules[0].label, equals(_l10n.passwordRuleLength(8)));
      // Rule 1 — digit
      expect(rules[1].label, equals(_l10n.passwordRuleDigit));
      // Rule 2 — uppercase
      expect(rules[2].label, equals(_l10n.passwordRuleUppercase));
    });
    ...
    test(
      ...
      () {
        final rules = passwordRules(_l10n, minLength: 12);
        expect(rules, hasLength(3));
        expect(rules[0].label, equals(_l10n.passwordRuleLength(12)));
      },
    );
  });
  ...
  group('PasswordChecklist widget', () {
    testWidgets('11. renders exactly 3 rows for the default 8-char factory', (
      ...
      await tester.pumpWidget(
        _wrap(PasswordChecklist(value: '', rules: passwordRules(_l10n))),
      );
    ...
    testWidgets('13. "Abcde123" satisfies all 3 rules → all rows show '
      ...
      await tester.pumpWidget(
        _wrap(
          PasswordChecklist(value: 'Abcde123', rules: passwordRules(_l10n)),
        ),
      );
    ...
    testWidgets(
      ...
      (tester) async {
        await tester.pumpWidget(
          _wrap(
            PasswordChecklist(value: 'ABCDEFGH', rules: passwordRules(_l10n)),
          ),
        );
        await tester.pumpAndSettle();
