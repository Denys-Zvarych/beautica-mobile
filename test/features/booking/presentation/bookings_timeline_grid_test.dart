...
void main() {
  ...
  group('window override', () {
    testWidgets('a window overrides the extent and hides out-of-window cards', (
      WidgetTester tester,
    ) async {
      final DateTime day = DateTime(2026, 7, 20);
      // In-window: 09:00 Kyiv == minute 540. Out-of-window: 20:00 Kyiv ==
      // minute 1200, well past the window's end.
      final Booking inWindow = _booking(
        id: 'in-window',
        startAtUtc: DateTime.utc(2026, 7, 20, 6), // 09:00 Kyiv
        durationMinutes: 60,
      );
      ...
      await tester.pumpApp(
        BookingsTimelineGrid(
          ...
          day: day,
          windowStartMinute: 480, // 08:00
          windowEndMinute: 720, // 12:00
          onBookingTap: (_) {},
        ),
      );
      await tester.pump();
      ...
      expect(
        find.byKey(const ValueKey<String>('timeline-card-in-window')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey<String>('timeline-card-out-of-window')),
        findsNothing,
      );
    });

    testWidgets(
      'the window bound is start-inclusive, end-exclusive, and only the '
      "booking's START minute (never its end) is checked",
      (WidgetTester tester) async {
        final DateTime day = DateTime(2026, 7, 20);
        // Window: minute 480 (08:00 Kyiv) .. 720 (12:00 Kyiv).
        // atStart: starts exactly ON the lower bound -> INCLUDED (inclusive).
        final Booking atStart = _booking(
          id: 'at-start',
          startAtUtc: DateTime.utc(2026, 7, 20, 5), // 08:00 Kyiv, minute 480
          durationMinutes: 30,
        );
        // atEnd: starts exactly ON the upper bound -> EXCLUDED (exclusive).
        final Booking atEnd = _booking(
          id: 'at-end',
          startAtUtc: DateTime.utc(2026, 7, 20, 9), // 12:00 Kyiv, minute 720
          durationMinutes: 30,
        );
        // spansPastEnd: STARTS inside the window (13:30, minute 810 of a
        // separate 09:00-14:00/540-840 window below) but ENDS after the
        // window closes (14:30) -> still INCLUDED, because membership is
        // decided by the start minute only, never the end minute.
        final Booking spansPastEnd = _booking(
          id: 'spans-past-end',
          startAtUtc: DateTime.utc(2026, 7, 20, 10, 30), // 13:30 Kyiv
          durationMinutes: 60, // ends 14:30 Kyiv
        );

        await tester.pumpApp(
          BookingsTimelineGrid(
            bookings: <Booking>[atStart, atEnd],
            day: day,
            windowStartMinute: 480, // 08:00
            windowEndMinute: 720, // 12:00
            onBookingTap: (_) {},
        ...
        expect(
          find.byKey(const ValueKey<String>('timeline-card-at-start')),
          findsOneWidget,
          reason: 'a booking starting exactly at the window start is IN',
        );
        expect(
          find.byKey(const ValueKey<String>('timeline-card-at-end')),
          findsNothing,
          reason:
              'a booking starting exactly at the window end is OUT '
              '(the upper bound is exclusive)',
        );

        await tester.pumpApp(
          BookingsTimelineGrid(
            bookings: <Booking>[spansPastEnd],
            day: day,
            windowStartMinute: 540, // 09:00
            windowEndMinute: 840, // 14:00
            onBookingTap: (_) {},
          ),
        );
        await tester.pump();
        ...
        expect(
          find.byKey(const ValueKey<String>('timeline-card-spans-past-end')),
          findsOneWidget,
          reason:
              'a booking starting inside the window renders even if its '
              'duration carries it past the window end',
        );
      },
    );
  });
